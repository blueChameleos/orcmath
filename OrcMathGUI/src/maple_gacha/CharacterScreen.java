...
public class CharacterScreen extends FullFunctionScreen {
	...
	@Override
	public void initAllObjects(List<Visible> viewObjects) { // this runs before beginnerselection updates the team
															// class.
				...
				public void act() {
					...
					if (idx != number && (clickG.get(number).getHero() != null)) {
						...
						confirmation.setText("Rank = " + clickG.get(number).getHero().getRank() + "\n" + "Strength = "
								+ clickG.get(number).getHero().getStrength() + "\n" + "Speed = "
								+ clickG.get(number).getHero().getSpeed() + "\n" + "Attack = "
								+ clickG.get(number).getHero().getAttack() + "\n" + "Defense = "
								+ clickG.get(number).getHero().getDefense() + "\n" + "HP = "
								+ clickG.get(number).getHero().getHP());
					}
				}
				...
				public void act() {
					...
					if (arrSize == 0) {
						if (clickList.get(number).getHero() != null
								&& !MainGame.team.get(findEquality(clickList.get(number).getHero())).isClickE()) {
